import { Tier1Check, Tier1Summary, RedFlag, CheckResult } from './types.js';
...
export const CHECK_MESSAGES: Record<
  string,
  Record<CheckResult, { factor: string; weight: 'positive' | 'negative' | 'neutral' }>
> = {
  '501c3_status': {
    PASS: { factor: '501(c)(3) tax-exempt status verified', weight: 'positive' },
    REVIEW: { factor: '501(c)(3) status needs verification', weight: 'neutral' },
  ...
  revenue_range: {
    PASS: { factor: 'Revenue in target range ($100K-$10M)', weight: 'positive' },
...
export function generateSummary(
  ...
  for (const check of checks) {
    ...
    if (checkConfig) {
      ...
      const prefix = msg.weight === 'positive' ? '+' : msg.weight === 'negative' ? '-' : '~';
  ...
  const issuesSummary =
    issues.length > 0 ? `Key concerns: ${issues.join('; ')}.` : 'No specific concerns identified.';

  // Interpolate template
  const justification = config.template
    .replace('{{score}}', String(score))
    .replace('{{name}}', name)
    .replace('{{years}}', yearsOperating !== null ? String(yearsOperating) : 'unknown')
